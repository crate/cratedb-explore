...
| --- | --- |
| [`src_ml/`](src_ml/README.md) | Predictive-maintenance pipeline over the `rtia` schema — training scripts plus a FastAPI real-time inference service; writes `rtia.fault_predictions`. See `src_ml/ML_GUIDE.md`. |
| [`src_mcp_search_rtia/`](src_mcp_search_rtia/README.md) | Single-file `FastMCP` server exposing `query_sql` over the `rtia` schema (HTTP `_sql`) plus four tools that proxy the `src_ml` inference service for live ML scoring. |
