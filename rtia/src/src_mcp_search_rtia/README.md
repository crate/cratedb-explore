...
```bash
cd rtia/src/src_mcp_search_rtia
python -m venv .venv
...
```bash
claude mcp add rtia -- python /abs/path/to/rtia/src/src_mcp_search_rtia/rtia_mcp.py
```
...
```json
{
  "mcpServers": {
    "rtia": {
      "command": "python",
      "args": ["/abs/path/to/rtia/src/src_mcp_search_rtia/rtia_mcp.py"]
    }
  }
}
```
