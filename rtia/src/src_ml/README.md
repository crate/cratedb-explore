...
From `rtia/src/src_ml/`, create a virtualenv and install the dependencies:
...
# rtia.fault_predictions table from rtia/sql/rtia_schema_create.sql — same columns,
...
`rtia/sql/rtia_schema_create.sql` (it carries the canonical DDL). The Python scorers
...
**File:** `realtime_inference.py` (in `rtia/src/src_ml/`)
...
rest of the `rtia` schema — are visualised by `rtia/grafana/rtia.json`, the **"Real
