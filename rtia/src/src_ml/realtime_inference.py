...
async def lifespan(app: FastAPI):
    ...
    # Predictions table must already exist (see rtia/sql/rtia_schema_create.sql)
    try:
...
def _require_predictions_table():
    ...
    rtia/sql/rtia_schema_create.sql, not here — fail with a clear message if missing
    ...
    if not exists:
        raise RuntimeError(
            ...
            'create the schema first:  crash < rtia/sql/rtia_schema_create.sql'
        )
