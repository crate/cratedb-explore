...
- Issues `SELECT min/max(data['temperature']) … WHERE distance(geo_location, %s::geo_point) < 1 AND measurement_time = %s` — one row out per call (min and max of a single grid cell at a single moment).
...
- Requires the `geo_points` and `timestamps` pools (`load_geo_points` / `load_timestamps` at startup).
...
- Issues `SELECT region_name, _score FROM german_regions WHERE MATCH(economics, %s) ORDER BY _score DESC LIMIT 3`.
