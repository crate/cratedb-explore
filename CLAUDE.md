...
| `src_stream_load/` | A producer that streams the three demo JSON datasets (from S3) into Kafka as JSON/Avro/Protobuf, and a consumer that reads them back out of Kafka into CrateDB. Reference tables (`german_regions`, `geo_points`) load first and in full; `climate_data` loads last and is rate-limitable (producer) / optionally tailable (consumer). The producer's destination sits behind a `StreamSink` ABC (`sinks.py`) so a non-Kafka platform can be swapped in. | `stream_load_into_kafka.py`, `stream_from_kafka_into_crate.py`, `sinks.py`, `serializers.py`, `schemas/` |
